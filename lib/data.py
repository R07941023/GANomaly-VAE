...
def load_data(opt):
    ...
    if opt.dataset in ['cifar10']:
        ...
        # if opt.isTrain:
        #     val_PASS_path = os.path.join(opt.dataroot, "val_PASS", "image")
        #     val_NG_path = os.path.join(opt.dataroot, "val_NG", "image")
        #     try:
        #         shutil.rmtree(val_NG_path)
        #     except:
        #         pass
        #     os.makedirs(val_NG_path)
        #     get_val_NG_dataset(opt, val_PASS_path, val_NG_path)


        folder_name = {'train': "train", 'val': 'val', 'test_PASS': "test_PASS", 'test_NG': "test_NG"}
        splits = ['train', 'val', 'test_PASS', 'test_NG']
        drop_last_batch = {'train': True, 'val':False, 'test_PASS': False, 'test_NG': False}
        shuffle = {'train': True, 'val': True, 'test_PASS': False, 'test_NG': False}
        transform = transforms.Compose([transforms.CenterCrop(opt.isize-opt.w_val_NG),
                                        transforms.Resize(opt.isize),  # opt.isize
                                        transforms.ToTensor(),
                                        transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5)), ])


        dataset = {x: ImageFolder(os.path.join(opt.dataroot, folder_name[x]), transform) for x in splits}
        dataloader = {x: torch.utils.data.DataLoader(dataset=dataset[x],
                                                     batch_size=opt.batchsize,
                                                     shuffle=shuffle[x],
                                                     num_workers=int(opt.workers),
...
##
# def get_val_NG_dataset(opt, PASS_path, NG_path, type_set=['.jpg', 'JPG', 'png', 'PNG']):
#     w = opt.w_val_NG
#     files = os.listdir(PASS_path)
#     for file in files:
#         if os.path.splitext(file)[1] in type_set:
#             img = cv2.imread(PASS_path + "/" + file)
#             roi_range = [int(img.shape[1]/2)-int(w/2), int(img.shape[0]/2)-int(w/2), int(img.shape[1]/2)+int(w/2), int(img.shape[0]/2)+int(w/2)]
#             img[roi_range[0]:roi_range[2], roi_range[1]:roi_range[3], :] = 0
#             cv2.imwrite(NG_path+"/"+file, img)
